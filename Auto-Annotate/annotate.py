...
def create_sub_mask_annotation(
    sub_mask, bounding_box, annotationId, classId, image_name
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", metavar="<command>", help="'annotateCoco' or 'annotateCustom'"
    )
    parser.add_argument(
        "--image_directory",
        required=True,
    ...
    if args.command == "annotateCoco":
        assert (
            args.label in COCO_DATASET_LABELS
        ), "Label --label does not belong to COCO labels "
    ...
    if args.command == "annotateCoco" or args.command == "annotateCustom":
        ...
        annotateImagesInDirectory(
            ...
            label=args.label,
            multiple_annotation=multiple_annotation,
        )
    else:
        print(
            "'{}' is not recognized. "
    ...
    with open("multiple_labels", "w") as filehandle:
        json.dump(multiple_annotation, filehandle)
