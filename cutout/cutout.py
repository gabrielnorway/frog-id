    ...
    for root, dir, files in os.walk(path):
        for file in files:
            if file.endswith(".json"):
                json = file
                dir_list.append(json[: len(json) - 5])
    return dir_list
...
if __name__ == "__main__":
    ...
    path_single_image_directory = (
        "/home/nos/code-school/bachelor-ikt300/Auto-Annotate/all_images/"
    )

    # path_single_image_directory = (
    #     os.path.join(__location__,"frog_id_1/")
    # )
    path_all_image_directory = path_single_image_directory
    target_image = "locN_2011_spring_0562_0178"

    matches = []
    images_list = []

    first_image_contour = json_read_contour(
        os.path.join(path_single_image_directory, target_image)
    )
    frog_list = read_json_directory(path_all_image_directory)
    ...
    for json_file in range(len(frog_list)):
        ...
        #
        # second_image_contour = json_read_contour(
        #     os.path.join(path_all_image_directory, frog_list[json_file])
        # )
        second_image = os.path.join(
            path_all_image_directory, frog_list[json_file] + ".jpg"
        )

        region_path = os.path.join(
            path_all_image_directory, frog_list[json_file] + "_region.jpg"
        )
    ...
    print(f"program took {stop - start:0.4f} seconds")
    for i in range(10):
